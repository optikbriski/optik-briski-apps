-- =============================================================================
-- DP → PENDING → CLEAR: selaraskan get_invoice_hub
-- - QR DP hanya di SIAP_PELUNASAN (bukan token orphan di PENDING_PO)
-- - Payload OBRINV + channel ONLINE|OFFLINE
-- - Return channel/fulfillment untuk Hub
-- =============================================================================
...
comment on function public.get_invoice_hub(text, text) is
  'Hub: DP QR = SIAP_PELUNASAN; LUNAS QR = SIAP_DIAMBIL/CLEAR; payload + channel.';
