-- Voucher ongkir OBR + diskon/cashback produk di online_orders.

alter table public.online_orders
  add column if not exists shipping_voucher_discount bigint not null default 0,
  add column if not exists product_promo_code text,
  add column if not exists product_promo_discount bigint not null default 0;
...
begin
  ...
  -- Diskon produk (clamp ke subtotal); voucher ongkir hanya bila delivery.
  v_prod_disc := greatest(0, coalesce(p_product_promo_discount, 0));
  if v_prod_disc > v_subtotal then
    v_prod_disc := v_subtotal;
  end if;
